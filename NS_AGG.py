...
class NetSchema(nn.Module):
    ...
    def forward(self, nei_h, nei_index):
        ...
        for i in range(self.nei_num):
            ...
            one_type_emb = (self.intra[i](sele_nei, nei_h[i + 1], nei_h[0]))
            embeds.append(one_type_emb)
        z_mc = self.inter(embeds)
        z_mc = F.elu(self.feat_drop(self.predict_1(z_mc)))
        return z_mc

class GCN(nn.Module):
    def __init__(self, hid_dim, length=3, bias=True):
    ...
    def forward(self, inputs, adj):
        ...
        for i in range(len(adj)):
            output = torch.spmm(torch.tensor(adj[i]), torch.spmm(inputs, self.weight[i]))  # AHW
            if self.bias[i] is not None:
